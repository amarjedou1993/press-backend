 ...
 *
 * ═══════════════════════════════════════════════════════════════════════
 * ⚠️ ITS TWIN IS HonourImportService, AND THEY WERE COPIED ON PURPOSE.
 *
 * The archive handling, the Excel date conversion, the path-traversal guard
 * and the photo-by-identity join are identical, and every one of them was
 * hard-won: the header match that failed on an asterisk, the date cell that
 * is a NUMBER rather than a string. A FIX TO ONE BELONGS IN THE OTHER.
 *
 * Extracting a shared parser was the tidier option and was rejected: it would
 * have rewritten a tested importer that HAPA already relies on, to add a
 * feature that did not yet exist. The duplication is mostly of code that is
 * finished — the cheap kind.
 *
 * WHAT LEGITIMATELY DIFFERS: the column list, the table written to, and the
 * absence of an expiry column — see HEADERS below.
 * ═══════════════════════════════════════════════════════════════════════
 *
 ...
 */
@Service
public class InstitutionalImportService {
     ...
     */
    /*
     * ⚠️ NO EXPIRY COLUMN, AND NO GRANT REASON — the two differences from the
     * honour template.
     *
     * The expiry is the MINISTRY's to set, at the grant, for the whole batch —
     * the way a session sets it for a cohort. An institution choosing how long
     * its own staff's credentials last would be choosing the term of a
     * document it does not issue.
     *
     * And the grant reason is absent because employment IS the reason. An
     * honour card must say why it bypassed the commission; this one says so by
     * naming the institution that filed it.
     */
    private static final String[] HEADERS = {
            "Nom complet", "NNI / Passeport", "Date de naissance",
            "Lieu de naissance", "Fonction", "Catégorie", "Spécialité"
    };
    ...
    public InstitutionalImportService(InstitutionalCardRepository repository,
    ...
    public record ImportRow(
            ...
            /** The employee's role at the institution — printed on nothing,
             *  kept because the Ministry reads the roll before granting. */
            String jobTitle,
    ...
    public ParsedArchive parse(Long institutionId, MultipartFile archive) {
        List<String> fileErrors = new ArrayList<>();

        if (archive == null || archive.isEmpty()) {
            throw new InstitutionalImportException("Aucun fichier reçu.");
        }
        if (archive.getSize() > MAX_ARCHIVE_BYTES) {
            ...
            throw new InstitutionalImportException(
        ...
        try (ZipInputStream zip = new ZipInputStream(archive.getInputStream())) {
            ...
            throw new InstitutionalImportException("L'archive n'a pas pu être lue.");
        }
        ...
        List<ImportRow> rows = readWorkbook(institutionId, workbookBytes,
                photos.keySet(), fileErrors);

        int valid = (int) rows.stream().filter(ImportRow::valid).count();
        int withPhoto = (int) rows.stream()
                .filter(r -> r.valid() && r.hasPhoto()).count();

        log.info("INSTITUTIONAL_IMPORT_PREVIEW rows={} valid={} withPhoto={} photos={}",
    ...
    private List<ImportRow> readWorkbook(Long institutionId, byte[] bytes,
        ...
        try (InputStream in = new ByteArrayInputStream(bytes);
            ...
            /*
             * ⚠️ PER INSTITUTION, not globally.
             *
             * The same person may legitimately hold a candidate's card and an
             * institutional one — different provenances, different series.
             * What must not happen is two live C cards from one body, which
             * a partial unique index also refuses.
             */
            Set<String> alreadyHeld = identities.isEmpty()
                    ? Set.of()
                    : new HashSet<>(repository.findLiveIdentityNumbers(
                            institutionId, identities));
        ...
        } catch (InstitutionalImportException e) {
            throw e;
        } catch (Exception e) {
            log.warn("INSTITUTIONAL_IMPORT_WORKBOOK_FAILED", e);
            throw new InstitutionalImportException(
    ...
    private ImportRow toRow(int rowNumber, String[] cells,
        ...
        if (identity.isBlank()) {
            ...
            if (alreadyHeld.contains(identity)) {
                errors.add("une carte en cours existe déjà pour ce numéro "
        ...
        /*
         * ⚠️ NO EXPIRY IS PARSED HERE, unlike the honour importer.
         *
         * The Ministry sets it at the grant, for the whole batch. A filing
         * carries no term because a filing is not yet a card.
         */

        PressCategory category = categories.get(normalise(cells[5]));
        if (!cells[5].isBlank() && category == null) {
            warnings.add("catégorie « " + cells[5].trim() + " » inconnue, ignorée");
        }
        Specialisation specialisation = specialisations.get(normalise(cells[6]));
        if (!cells[6].isBlank() && specialisation == null) {
            warnings.add("spécialité « " + cells[6].trim() + " » inconnue, ignorée");
        }

        boolean hasPhoto = !identity.isBlank() && photoKeys.contains(identity);
        if (!hasPhoto && errors.isEmpty()) {
            // ⚠️ A WARNING, NEVER AN ERROR. The filing is created and waits;
            // the institution attaches the photograph afterwards, and only
            // then does the card reach the printer. Refusing the row would
            // force a second import for the same people.
        ...
        return new ImportRow(
                ...
                jobTitle.isBlank() ? null : jobTitle,
                hasPhoto,
