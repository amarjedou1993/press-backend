 ...
 */
@Service
public class InstitutionalImportTemplate {

    /**
     * ⚠️ THE SAME ORDER AS InstitutionalImportService.HEADERS, and the parser
     * reads BY POSITION. Changing one without the other silently maps every
     * column to its neighbour — names into identity numbers, dates into
     * job titles.
     *
     * ⚠️ NO EXPIRY COLUMN. The Ministry sets the term at the grant, for the
     * whole batch. An institution choosing how long its own staff's
     * credentials last would be choosing the term of a document it does not
     * issue.
     */
    ...
    public InstitutionalImportTemplate(PressCategoryRepository categoryRepository,
    ...
    private void buildEntrySheet(Workbook workbook) {
        ...
        titleCell.setCellValue("Agents de l'institution — import en masse");
        ...
        for (String line : List.of(
                "Remplissez une ligne par agent. Les colonnes marquées * sont obligatoires.",
                ...
                + "archive .zip (10 Mo maximum, une quarantaine d'agents).",
                "La DATE D'EXPIRATION n'est pas demandée : elle est fixée par le Ministère "
                + "au moment de l'octroi, pour l'ensemble du lot.",
                "Un agent sans photographie est tout de même déposé : sa carte attend "
                + "son image avant de pouvoir être produite.",
                "Catégories et spécialités : voir la feuille « Références ». "
                + "Une valeur inconnue est ignorée, la fiche est créée sans elle.")) {
