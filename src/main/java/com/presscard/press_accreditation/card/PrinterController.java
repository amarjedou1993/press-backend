...
import com.presscard.press_accreditation.institutional.InstitutionalArchiveService;
import com.presscard.press_accreditation.institutional.InstitutionalCard;
import com.presscard.press_accreditation.institutional.InstitutionalCardService;
...
public class PrinterController {
    ...
    public record RunSummary(
            ...
            Long sessionId,
            String sessionLabel,
            /** HOW the cards left: ASSETS or PDF. */
            String kind,
            /**
             * WHICH SERIES this run produced: CARD, HONOUR or INSTITUTIONAL.
             *
             * ───────────────────────────────────────────────────────────
             * ⚠️ NOT THE SAME QUESTION AS `kind`, AND THE HISTORY WAS
             * UNREADABLE WITHOUT IT.
             *
             * All three series leave this building as ASSETS — that is what
             * `kind` records. So an honour run, an institutional run and a
             * session run rendered as three identical lines: "3 cartes ·
             * ressources de production", with only a session label
             * distinguishing the first, and only when one was passed.
             *
             * ⚠️ DERIVED FROM THE RUN'S ROWS, not stored on the run. The
             * series is already on the card each row points at, and a second
             * copy would eventually disagree — which is also why
             * print_runs.kind was left alone. See seriesByRunIds.
             * ───────────────────────────────────────────────────────────
             */
            String series,
            int cardCount
    ...
    /**
     * An institutional card, as the producer reads it.
     *
     * ⚠️ IT CARRIES ITS BODY, and that is the difference from an honour card.
     * A run for one institution is a different physical batch from a run for
     * another — different envelopes, different collection — so the producer
     * works one body at a time.
     */
    public record PrintableInstitutionalCard(
            Long cardId,
            String cardNumber,
            String holderFullName,
            String jobTitle,
    ...
    /** One body's producible cards, grouped as the screen shows them. */
    public record PrintableInstitutionGroup(
            Long institutionId,
            String institutionNameFr,
            List<PrintableInstitutionalCard> cards
    ) {}

    public record InstitutionalArchiveRequest(
            @NotEmpty(message = "Sélectionnez au moins une carte.")
            List<Long> cardIds
    ) {}
    ...
    /**
     * ⚠️ Read for the history's series column, and nothing else.
     *
     * A run cannot say which series it produced; its rows can, by which card
     * column they set — which the one-provenance CHECK makes unambiguous.
     */
    ...
    private final InstitutionalCardService institutionalCardService;
    ...
    /**
     * Institutional cards awaiting production, GROUPED BY BODY.
     *
     * ───────────────────────────────────────────────────────────────────
     * ⚠️ GROUPED, UNLIKE THE HONOUR TAB — and the reason is physical.
     *
     * An honour card belongs to no cohort: granted one at a time, on its own
     * occasion, so a flat list is the honest shape.
     *
     * An institution's cards are a batch. They are collected together, by one
     * body, in one envelope. A producer making forty cards for one agency and
     * six for another is making two runs, and a flat list would leave them to
     * reconstruct that from the names.
     *
     * The institution is to a C card what a session is to an A card: the
     * thing that says which batch it belongs to.
     * ───────────────────────────────────────────────────────────────────
     */
    @GetMapping("/institutional-cards")
    @Transactional(readOnly = true)
    public List<PrintableInstitutionGroup> institutionalCards() {
        List<InstitutionalCard> cards = institutionalCardService.producible();
        if (cards.isEmpty()) {
            return List.of();
        }
        ...
        // One query for the whole list, as for the other two series.
        Map<Long, Long> counts = runRepository
                .countByInstitutionalCardIds(
                        cards.stream().map(InstitutionalCard::getId).toList())
                .stream()
                .collect(Collectors.toMap(row -> (Long) row[0], row -> (Long) row[1]));

        return cards.stream()
                .collect(Collectors.groupingBy(
                        InstitutionalCard::getInstitutionId,
                        LinkedHashMap::new,
                        Collectors.toList()))
                .entrySet().stream()
                .map(entry -> {
                    Institution institution = institutions.get(entry.getKey());
                    String name = institution == null ? "—" : institution.getNameFr();

                    return new PrintableInstitutionGroup(
                            entry.getKey(),
                            name,
                            entry.getValue().stream().map(card -> {
                                PressCategory category = card.getCategoryId() == null
                                        ? null : categories.get(card.getCategoryId());
                                return new PrintableInstitutionalCard(
                                        card.getId(),
                                        card.getCardNumber(),
                                        card.getFullName(),
                                        card.getJobTitle(),
                                        category == null ? "—" : category.getLabelFr(),
                                        card.getInstitutionId(),
                                        name,
                                        card.getIssuedAt(),
                                        card.getExpiresAt(),
                                        counts.getOrDefault(card.getId(), 0L));
     ...
     *
     * ⚠️ PHOTOGRAPH AND QR ONLY, as for honour cards. CardPdfService lays out
     * a card from a dossier, and these have none — which suits the
     * requirement and leaves one fewer artefact carrying the Ministry's
     * layout out of the building.
     */
    @PostMapping("/institutional-archive")
    public ResponseEntity<byte[]> institutionalArchive(
            @Valid @RequestBody InstitutionalArchiveRequest request,
            Principal principal) {

        Long actorId = actorId(principal);

        // ⚠️ RE-VALIDATED SERVER-SIDE, as for the other two series: the screen
        // offers only producible cards, but the request carries ids — and one
        // suspended since the page was opened must not be produced.
        List<Long> producible = institutionalCardService.producible().stream()
                .map(InstitutionalCard::getId)
                .filter(request.cardIds()::contains)
                .toList();
        ...
        InstitutionalArchiveService.ArchiveResult result =
                institutionalArchiveService.archive(producible);

        // Recorded AFTER the file exists, into the SAME history as ordinary
        // and honour production.
        printRunService.recordInstitutional(actorId, producible);

        return ResponseEntity.ok()
                .contentType(MediaType.parseMediaType("application/zip"))
                .header(HttpHeaders.CONTENT_DISPOSITION,
                        "attachment; filename=\"cartes-institutionnelles-%s.zip\""
    ...
    public List<RunSummary> history(Principal principal,
        ...
        /*
         * ⚠️ ONE QUERY FOR THE WHOLE PAGE, like the actor's name above.
         *
         * A lookup per run would be fifty queries on a screen that exists to
         * be scanned — the same correction the other methods here already
         * carry.
         */
        Map<Long, String> series = runCardRepository
                .seriesByRunIds(runs.stream().map(PrintRun::getId).toList())
                .stream()
                .collect(Collectors.toMap(row -> (Long) row[0], row -> (String) row[1]));

        return runs.stream()
                .map(run -> toSummary(run, actorName, sessions,
                        // ⚠️ CARD for a run with no rows at all — one recorded
                        // before print_run_cards existed, which the history
