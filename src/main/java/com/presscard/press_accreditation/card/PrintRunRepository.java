...
public interface PrintRunRepository extends JpaRepository<PrintRun, Long> {
    ...
    /** Whether this institutional card has ever been produced. */
    @Query("""
           SELECT COUNT(prc) > 0 FROM PrintRunCard prc
           WHERE prc.institutionalCardId = :cardId
           """)
    boolean institutionalCardWasProduced(@Param("cardId") Long cardId);

    /**
     * How many runs have included each of these cards.
     *
     * ⚠️ ONE QUERY FOR THE WHOLE LIST — the same correction made to the
     * honour tab, and to four other controllers before it.
     */
    @Query("""
           SELECT prc.institutionalCardId, COUNT(prc)
           FROM PrintRunCard prc
           WHERE prc.institutionalCardId IN :cardIds
